...
class NikaDevsCmsController extends ControllerBase {
  ...
  public function release_cleanup() {
    ...
    db_truncate('cache_data')->execute();
    db_truncate('cache_default')->execute();
    db_truncate('cache_entity')->execute();
    db_truncate('cache_menu')->execute();
    db_truncate('cache_render')->execute();
    db_truncate('cache_rest')->execute();
    db_truncate('cache_toolbar')->execute();
    db_truncate('watchdog')->execute();

    db_update('users_field_data')->fields(
      array(
        'name' => 'Admin',
